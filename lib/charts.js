...
// Atlas: the canvas is width*dpr x height*dpr, so width/height caps alone do
// not bound memory (w=1200&devicePixelRatio=10 is a 576 MB canvas). Cap the
// output pixels and the number of renders in flight instead.
const MAX_PIXELS = parseInt(process.env.CHART_MAX_PIXELS, 10) || 16000000; // 4000x4000, ~64 MB
const MAX_CONCURRENT = parseInt(process.env.CHART_MAX_CONCURRENT, 10) || 4;
let inFlight = 0;
const waiting = [];
function acquire() {
...
async function getChartJsForVersion(version) {
  if (version && version.startsWith('4')) {
    // Atlas patch: load the CommonJS build so the plugins (which `require('chart.js')`)
    // share one Chart instance with the renderer; register all controllers like /auto does.
    const { Chart, registerables } = require('chart.js-v4');
...
async function renderChartJs(
  ...
  if (typeof untrustedChart === 'string') {
    // Atlas hardening: the config is parsed as JSON only. JavaScript configs
    // (functions, unquoted keys, gradient/pattern helpers) are disabled on this
    // instance because `new Function` on URL input is remote code execution.
    try {
      chart = JSON.parse(untrustedChart);
    } catch (err) {
      logger.info('Rejected non-JSON chart config');
      const inputError = new Error(
        'Invalid input\nChart config must be valid JSON (JavaScript configs are disabled on this instance)',
      );
  ...
  // in absolute terms.
  const dpr = parseFloat(devicePixelRatio) || 2.0;
  if (!(dpr > 0) || width * height * dpr * dpr > MAX_PIXELS) {
    return Promise.reject(
      badInput(`Output too large: width*height*devicePixelRatio^2 must be <= ${MAX_PIXELS} pixels`),
    );
  }
  ...
  if (!chart.plugins) {
    if (version.startsWith('4')) {
      // Atlas patch: Chart.js 4 needs the v4-compatible plugin builds. They must
      // be registered on the matching Chart class so their defaults are applied.
      const datalabelsV4 = require('chartjs-plugin-datalabels-v4');
      const annotationV4 = require('chartjs-plugin-annotation-v4');
      require('chart.js-v4').Chart.register(datalabelsV4, annotationV4);
      chart.plugins = [datalabelsV4, annotationV4];
    } else if (version.startsWith('3')) {
      const datalabelsV3 = require('chartjs-plugin-datalabels-v3');
      const annotationV3 = require('chartjs-plugin-annotation-v3');
      require('chart.js-v3').Chart.register(datalabelsV3, annotationV3);
      chart.plugins = [datalabelsV3, annotationV3];
    } else {
